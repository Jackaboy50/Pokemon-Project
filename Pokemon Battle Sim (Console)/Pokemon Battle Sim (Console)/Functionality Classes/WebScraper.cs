...
namespace Pokemon_Battle_Sim__Console_
{
    static class WebScraper
    {
        ...
        public static void GetFeaturedGames(string PKMN, out List<string> featuredGames, out List<string> localDexNumbers)
        {
            Functions fn = new Functions();
            featuredGames = new List<string>();
            localDexNumbers = new List<string>();
            int div = 1;
            string[] regionalCheck = PKMN.Split(" ");
            if (regionalCheck.Contains("A") || regionalCheck.Contains("G") || regionalCheck.Contains("H") || regionalCheck.Contains("P"))
            {
            ...
            HtmlWeb web = new HtmlWeb();
            var doc = web.Load(url);
            var nodes = doc.DocumentNode.SelectNodes($"/html/body/main/div[2]/div[2]/div[{div}]/div[1]/div[2]/table/tbody/tr[7]/td/small[position()>0]");
            if(nodes == null)
            {
                return;
            }
            foreach(var node in nodes)
            {
                ...
                {
                    case "(Sun/Moon &mdash; Alola dex)":
                        featuredGames.Add("Sun/Moon - Alola dex");
                        break;

                    case "(U.Sun/U.Moon &mdash; Alola dex)":
                        featuredGames.Add("U.Sun/U.Moon - Alola dex");
                        break;

                    case "(X/Y &mdash; Central Kalos)":
                        featuredGames.Add("X/Y - Central Kalos");
                        break;

                    case "(Yellow/Red/Blue)":
                        featuredGames.Add("Red/Blue/Yellow");
                        break;

                    default:
                        featuredGames.Add(node.InnerText.Trim(new char[] { '(', ')' }));
                        break;
                }
            }
            nodes = doc.DocumentNode.SelectNodes($"/html/body/main/div[2]/div[2]/div[{div}]/div[1]/div[2]/table/tbody/tr[7]/td/text()[position()>0]");
            foreach(var node in nodes)
            {
                localDexNumbers.Add(node.InnerText);
            }
            return;
        }
        ...
        public static Pokemon GetPokemon(string PKMN)
        {
            ...
            List<string> FG;
            List<string> LDN;
            GetFeaturedGames(PKMN, out FG, out LDN);
            ...
            return new Pokemon( $"{regionalName} {regionalCheck[0]}", T1, T2, B, PM, FG, LDN);
